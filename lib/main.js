...
export class TreeUtils {
  ...
  // 若节点和它的所有后代节点都不满足条件, 则它和它的所有后代节点都从结果中排除
  search (tree, fn) {
    return tree?.map(node => {
  ...
  flat (tree) {
    return tree?.reduce((res, node) => res.concat(node, this.flat(node[this.children])), [])
  }
}
