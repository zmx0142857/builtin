...
/**
 * 在数组中查找一项, 若找不到则插入
 * TIPS: 对于 object, 类似的代码是 `item = obj[key] ??= defaultValue`
 */
export const findOrInsert = (arr, fn, defaultValue) => {
  return arr.find(fn) ?? (arr.push(defaultValue), defaultValue)
}
...
export const set = (obj, path, value) => {
  if (typeof path === 'string') path = path.split('.')
  obj = path.slice(0, -1).reduce((cur, key, i) => {
    return cur[key] ??= /^[0-9]|[1-9][0-9]+$/.test(path[i+1]) ? [] : {}
    return cur instanceof Object ? cur[key] : undefined
  }, obj)
