 ...
 * @param {number} options.level jpeg 质量
 * @returns {Promise<string>} base64 图片
 */
export const compressImage = async (src, { width = 0, height = 0, level = 0.8 } = {}) => {
  const canvas = document.createElement('canvas')
  const ctx = canvas.getContext('2d')
  if (!ctx) throw new Error('[compressImageBase64] ctx is null')
