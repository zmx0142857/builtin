 ...
 * ```
 * TIPS: 也可以使用 lodash.uniq 和 lodash.uniqBy
 */
export const uniq = (arr, fn) => {
  // fromEntries 不能保持数组顺序, 如 ['01', '02', '10', '11']
  // if (fn) return Object.values(Object.fromEntries(arr.map(v => [fn(v), v])))
  if (fn) return [...new Map(arr.map(v => [fn(v), v])).values()]
  return [...new Set(arr)]
}
 ...
 * @example
 * ```
 * rename({ a: 1, b: 2, c: 3 }, { x: 'a', y: 'b' }) // { a: 1, b: 2, c: 3, x: 1, y: 2 }
 * ```
 */
export const rename = (obj, dict, options = {}) => {
  const { copy = true, keep = true, recursive = false, reverse = false } = options
  if (!copy) obj = { ...obj }
  Object.entries(dict).forEach(([newKey, oldKey]) => {
    if (newKey === oldKey) return
    if (reverse) [newKey, oldKey] = [oldKey, newKey]
    if (typeof oldKey === 'function') {
      obj[newKey] = oldKey(obj)
    } else {
      obj[newKey] = obj[oldKey]
      if (!keep) delete obj[oldKey]
    }
    if (recursive) obj[recursive === true ? 'children' : recursive]?.forEach(v => rename(v, dict, options))
  })
  return obj
}
 ...
 * @param {object} options
 * @param {string} options.format 格式串, 默认 YYYY-MM-DD HH:mm:ss
 * @param {boolean} options.local 是否使用本地时间, 默认 true
 * @example
 * ```
 * formatDate() // '2026-07-24 10:06:22'
 * formatDate({ format: 'Y-M-D H:m:s' }) // '2026-7-24 10:6:22'
 * ```
 */
export const formatDate = ({ date = new Date(), format = defaultFormat, local = true } = {}) => {
  if (local) date = new Date(date - date.getTimezoneOffset() * 60000)
  const str = date.toISOString()
  ;'YYYY,YY,Y,MM,M,DD,D,HH,H,mm,m,ss,s'.split(',').forEach(key => {
    const last = defaultFormat.lastIndexOf(key)
    const len = key.length
    const value = len === 1 ? Number(str.slice(defaultFormat.indexOf(key), last + 1)) : str.substr(last, len)
 ...
 * @example
 * ```
 * joinPath('http://aaa/', '/bbb/', '/ccc/') => 'http://aaa/bbb/ccc/'
 * ```
 */
