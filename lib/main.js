 ...
 * @example
 * ```
 * rename({ a: 1, b: 2, c: 3 }, { a: 'x', b: 'y' }) // { c: 3, x: 1, y: 2 }
 * ```
 */
export const rename = (obj, dict, { inplace = false, keepOld = false } = {}) => {
  if (!inplace) obj = { ...obj }
  Object.entries(dict).forEach(([oldKey, newKey]) => {
    if (!newKey || oldKey === newKey || obj[oldKey] === undefined) return
    obj[newKey] = obj[oldKey]
    if (!keepOld) delete obj[oldKey]
  })
  return obj
}
