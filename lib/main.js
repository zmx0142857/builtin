...
export const clone = (obj, level = Infinity) => {
  ...
  if (Array.isArray(obj)) return obj.map(v => clone(v, level-1))
 ...
 * @returns {HTMLElement}
 */
export const div = ({ el, tag = 'div', container = document.body, selector, className, css, style, attr, on, off, namespace, ...options } = {}) => {
  if (selector === true) selector = '.' + className
  el ||= (selector && container.querySelector(selector)) || container.appendChild(
  ...
  if (attr) each(attr, (v, k) => el.setAttribute(k, v))
  if (on) each(on, (v, k) => el.addEventListener(k, v))
  if (off) each(off, (v, k) => el.removeEventListener(k, v))
