 ...
 * @param {{x: number, y: number, rotate: number, scale: number}} transform 初始位置
 */
export const Draggable = ({
  ...
  render ||= (trans, delay = 0) => {
    const { x, y, rotate, scale } = trans
    el.style.transition = delay ? `transform ${delay}ms` : ''
  ...
  const snapToEdge = () => {
    ...
    trans.x = between(x, 0, rect.width - scale * w, snap)
    trans.y = between(y, 0, rect.height - scale * h, snap)
    render(trans, delay)
  }
  ...
  const multiTouch = (e) => {
    ...
    render(trans)
    clearTimeout(timer)
    timer = setTimeout(snapToEdge, delay / 10)
  }
...
export const Canvas = ({ container, dpr = window.devicePixelRatio } = {}) => {
  const canvas = div({
    container,
    tag: 'canvas',
    style: {
