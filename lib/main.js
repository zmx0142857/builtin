 ...
 */
export const formatDate = ({ date = new Date(), format = 'YYYY-MM-DD HH:mm:ss', local = true } = {}) => {
  if (local) date = new Date(date - date.getTimezoneOffset() * 60000)
  const str = date.toISOString()
  return format.replace('YYYY', str.slice(0, 4))
    .replace('YY', str.slice(2, 4))
    .replace('Y', Number(str.slice(0, 4)))
    .replace('MM', str.slice(5, 7))
    .replace('M', Number(str.slice(5, 7)))
    .replace('DD', str.slice(8, 10))
    .replace('D', Number(str.slice(8, 10)))
    .replace('HH', str.slice(11, 13))
    .replace('H', Number(str.slice(11, 13)))
    .replace('mm', str.slice(14, 16))
    .replace('m', Number(str.slice(14, 16)))
    .replace('ss', str.slice(17, 19))
    .replace('s', Number(str.slice(17, 19)))
}
