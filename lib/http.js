import { matchString, rename, sleep } from './main'
...
export const createHttp = (props) => ({
  ...
  async fetchByWx (url, req) {
    if (req.headers['Content-Type'] === contentTypeEnum.multipart) {
      const raw = await new Promise((success, fail) => wx.uploadFile({
        url,
        method,
        header: req.headers,
        success,
    ...
    const raw = await new Promise((success, fail) => wx.request({
      url,
      method: req.method,
      header: req.headers,
      data: req.body,
